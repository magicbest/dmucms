	...
	<body>
        ...
        <div align="right" style="margin-right:20px;margin-bottom: -15px;"   id="prolist" > </div>  
			<div class="container-fluid">
								...
								<table class="table table-bordered table-striped">
									...
									<tbody>
										<tr>
											...
											<td align="center">
											  <c:if test="${project.projectRank == null}">暂未定级</c:if> 
											  <c:if test="${project.projectRank != null}">${project.projectRank}</c:if>
											 </td>
											<td>
											  <c:if test="${project.projectCost == null}">待定</c:if> 
										   ...
										   <td>
										       <a href="#passAlert1" data-toggle="modal" class="btn btn-success btn-mini">查看</a>
										   </td>
											<td>
											   <a href="#passAlert2" data-toggle="modal" class="btn btn-success btn-mini">查看</a>
											</td>
											<td>
											       <a href="#passAlert3" data-toggle="modal" class="btn btn-success btn-mini">查看</a>
											</td>
										</tr>
									</tbody>
								</table>							
				...
				<div id="passAlert1" class="modal hide">
					<div class="modal-header">
						<button data-dismiss="modal" class="close" type="button"></button>
						<h3>指导教师评语：</h3>
					</div>
					<div class="modal-body">
						<textarea  readonly="readonly"  rows="3" name="teacherComment"  style="width:500px;">
						   <c:if test="${project.teacherComment == null}">未填写</c:if>
						   <c:if test="${project.teacherComment != null}">${project.teacherComment}</c:if>
						</textarea>
					</div>
              </div>
              
              <div id="passAlert2" class="modal hide">
					<div class="modal-header">
						<button data-dismiss="modal" class="close" type="button"></button>
						<h3>学院评语：</h3>
					</div>
					<div class="modal-body">
						<textarea  readonly="readonly"  rows="3" name="teacherComment"  style="width:500px;">
						  <c:if test="${project.collegeComment == null}">未填写</c:if>
						   <c:if test="${project.collegeComment != null}">${project.collegeComment}</c:if>
						</textarea>
					</div>
              </div>
              
              <div id="passAlert3" class="modal hide">
					<div class="modal-header">
						<button data-dismiss="modal" class="close" type="button"></button>
						<h3>中心评语：</h3>
					</div>
					<div class="modal-body">
						<textarea  readonly="readonly"  rows="3" name="teacherComment"  style="width:500px;">
						   <c:if test="${project.centerComment == null}">未填写</c:if>
						   <c:if test="${project.centerComment != null}">${project.centerComment}</c:if>
						</textarea>
					</div>
              </div>
